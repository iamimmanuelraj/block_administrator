        ...
        if ($('.block-administrator-wrapper').length === 0) {
            ...
            const field = frappe.ui.form.make_control({
                df: {
                    ...
                    description: "When checked, the inbuilt frappe's Administrator account is blocked from email-password login. When unchecked, the Administrator account can log in using email and password.",
                },
            ...
            if (!frm.block_administrator_login_cached) {
                frappe.db.get_single_value("Block Administrator", "block_administrator_login")
                    .then(value => {
                        ...
                        field.$input.on('change', function() {
                            ...
                            frappe.db.set_value('Block Administrator', 'Block Administrator', 'block_administrator_login', currentValue)
                                .then(() => {
                                    frappe.show_alert({
                    ...
                    .catch(err => {
                        console.error("Error fetching block_administrator_login value:", err);
                    });
            } else {
                // Use cached value
